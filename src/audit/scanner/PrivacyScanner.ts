...
export class PrivacyScanner extends AssuranceScanner {
    ...
    public _prepareTestPlan(pContext:DexcaliburProject):TestPlan{
        ...
        if(tests[TestType.IAST].length>0){
            plan.addStep(TestType.IAST, tests[TestType.IAST]);

            //if(tests[TestType.STATIC_SCAN].length>0)
            //    plan.addStep(TestType.STATIC_SCAN, tests[TestType.STATIC_SCAN]);
        }
    ...
    private async _firstScan(pContext:DexcaliburProject, pOptions:GenericScanOptions):Promise<void>{
        ...
        const plan = this._prepareTestPlan(pContext);
    ...
    override async runModel(pContext:DexcaliburProject):Promise<AssuranceReport>{
