...
export var Saver:InspectorFactory = new InspectorFactory({
    ...
    eventListeners: {
        "save.autosave.start": function(ctx:DexcaliburProject, event:Event):any{
            ctx.saveManager.enable();
        },
        "save.autosave.stop": function(ctx:DexcaliburProject, event:Event):any{
            ctx.saveManager.disable();
        },
        "dxc.fullscan.post": function(ctx:DexcaliburProject, event:Event):any{
        ...
        "field.alias.update": function(ctx:DexcaliburProject, event:Event):any{
            console.log(event);
            try{
                if(ctx.saveManager.isReady() && ctx.saveManager.isEnabled()){
        ...
        "probe.new": function(ctx:DexcaliburProject, event:Event):any{
            try{
                if(ctx.saveManager.isReady() && ctx.saveManager.isEnabled()){
        ...
        "probe.post_code_change": function(ctx:DexcaliburProject, event:Event):any{
            try{
                if(ctx.saveManager.isReady() && ctx.saveManager.isEnabled()){
                    ctx.saveManager.updateHook(event.data); 
                    ctx.saveManager.save();
        ...
        "probe.enable": function(ctx:DexcaliburProject, event:Event):any{
            try{
                if(ctx.saveManager.isReady() && ctx.saveManager.isEnabled()){
        ...
        "probe.disable": function(ctx:DexcaliburProject, event:Event):any{
            try{
                if(ctx.saveManager.isReady() && ctx.saveManager.isEnabled()){
