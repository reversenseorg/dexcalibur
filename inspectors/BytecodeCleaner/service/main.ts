...
function gotoLocalXref(method:ModelMethod, goto_name:string):any{
    let xref:any = {
        name: null,
        ref: [],
        ctr: 0
...
function checkIfEligible(method:ModelMethod):boolean{
...
function findSingleGoto(method:ModelMethod):string[]{
    let found:any = {}, singles:any = [];
    let instruction:ModelInstruction;
...
function nextSingleGoto(method:ModelMethod):any{
    let found:any = {}, singles:any = [];
    let instruction:ModelInstruction;
...
function isJump(instr:ModelInstruction):boolean{
    return instr !=null
        && instr.opcode != null
        && (instr.opcode.type==CONST.INSTR_TYPE.GOTO || instr.opcode.type==CONST.INSTR_TYPE.RET );
}
function hasJump(block:ModelInstruction[], label:null=null){
    let f = false;
    if(label == null)
...
function isReturn(instr:ModelInstruction):boolean{
    return instr !=null
        && instr.opcode != null
 ...
 * @param {*} gotoLabel
 */
function findTargetBasicBlocks(method:ModelMethod, gotoLabel:string):any{
...
function moveBasicBlock(method:any, bblocks:any, gotoLabel:any):any{
 ...
 * @return {boolean}
 */
function flatternGotoOf(method:ModelMethod){
 ...
 * @param {*} context
 */
function gotoConditionnalClean(context:DexcaliburProject){
...
function gotoClean(context:DexcaliburProject){
