...
describe('ADB Wrapper', function() {
    ...
    before(function(){
        // whatsapp_single_file


        // mock child_process.execSync()
        /*TestHelper.interceptExec( function(x){
            return (x.indexOf("shell pm path com.test.unit3")>-1);
        }, `package:/data/app/com.test.unit3-2/base.apk${EOL}package:/data/app/com.test.unit3-1/base.apk`);
    */
    })
    ...
    beforeEach(function() {
        ...
        }, `package:/data/app/com.unit_single_file-2/base.apk`);

        TestHelper.interceptExec( function(x){
            return (x.indexOf("shell pm path com.test.unit_mf")>-1);
        }, `package:/data/app/com.test.unit_mf-2/base.apk${EOL}package:/data/app/com.test.unit_mf-1/base.apk`);
    });
    ...
    describe('new ADB Wrapper', function() {
        ...
        it('new with device id', function () {
            ...
            let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH, "1a2b3c4d5e6f");
    ...
    describe('setup( [deviceID = null])', function() {
        ...
        it('with no device ID', function () {
            ...
            let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH);
            let cmd:string = adbw.setup(null,true) as string;
        ...
        it('device ID specified, but the wrapper initialized without default device ID', function () {
            ...
            let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH);
            let cmd:string = adbw.setup("aabbcceeff", true) as string;
    ...
    describe('getPackagePath(packageIdentifier, deviceId=null)', function() {
            ...
            it('single file', function(){
                ...
                expect(flag).to.equals(0);
                expect(ret).to.equals("/data/app/com.unit_single_file-2/base.apk");
            });
            ...
            it('single file, custom deviceID (false)', function(){
                let flag:number = 0, ret:any=null;
                let expectedDeviceId:string = "aabbccddeeff";
                let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH,expectedDeviceId);
                let dev:boolean = false;

                TestHelper.interceptExec( function(x){
                    if(x.indexOf("shell pm path com.test.unit_sf_cd")>-1){
                ...
                }, `package:/data/app/com.test.unit_sf_cd-2/base.apk`);

                
                try{
                    ret = adbw.getPackagePath(`com.test.unit_sf_cd`) ;
                }catch(err){
                    flag++;
                }
                ...
                expect(ret).to.equals("/data/app/com.test.unit_sf_cd-2/base.apk");
            });
            ...
            it('single file, default deviceID', function(){
                ...
                TestHelper.interceptExec( function(x){
                    if(x.indexOf("shell pm path com.unit_sf_dd")>-1){
                ...
                }, `package:/data/app/com.unit_sf_dd-2/base.apk`);

                
                try{
                    ret = adbw.getPackagePath(`com.unit_sf_dd`);
                }catch(err){
                    flag++;
                }
                ...
                expect(flag).to.equals(0);
                expect(dev).to.equals(true);
                expect(ret).to.equals("/data/app/com.unit_sf_dd-2/base.apk");
            });
            ...
            it('multiple files', function(){
                ...
                try{
                    ret = adbw.getPackagePath(`com.test.unit_mf`);
                }catch(err){
                    console.log(err);
                    flag++;
                }
                ...
                expect(flag).to.equals(0);
                expect(ret).to.equals("/data/app/com.test.unit_mf-2/base.apk");
            });
    });
    ...
    describe('parsePackageList( pPackageListStr)', function() {
        ...
        it('invalid ADB output : device/emulators not found', function () {
            ...
            let adbw:AdbWrapper = new AdbWrapper(process.cwd()+"/test/invalide_adb_path");
            let f:number = 0;
    ...
    describe('listPackages(deviceId = null)', function() {
        ...
        it('list packages', function () {
            ...
            let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH);
            let f:number = 0, ret:AppPackage[]=null;
    ...
    describe('push(local_path, remote_path, deviceID=null)', function() {
        ...
        it('push file, no default device, no specific device', function(){
            let flag:number = 0, ret:any=null, f:string;
            let adbw:AdbWrapper = new AdbWrapper(VALID_ADB_PATH);
